...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    if (logicalSize != 1) {
        int temp_size = logicalSize;

        while (temp_size != 1 && elems[temp_size].weight < elems[temp_size / 2].weight) {
            swap(elems[temp_size], elems[temp_size / 2]);
            temp_size /= 2;
        }
    }
}
...
DataPoint HeapPQueue::dequeue() {
    ...
    // Store the root (smallest element) to return later
    DataPoint ret = elems[1];

    // Replace the root with the last element and decrease logical size
    ...
    while (true) {
        ...
        if (leftChild <= logicalSize && elems[leftChild].weight < elems[smallest].weight) {
            smallest = leftChild;
        }

        // Compare with the right child
        if (rightChild <= logicalSize && elems[rightChild].weight < elems[smallest].weight) {
            smallest = rightChild;
        }

        // If the smallest is not the current element, swap and continue
        if (smallest != currentIndex) {
            swap(elems[currentIndex], elems[smallest]);
