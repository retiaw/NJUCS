...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    if (populations.size() > numSeats) {
        error("no enough Seats!");
    } else {
        HeapPQueue que;
        Map<string, int> seatsAssigned = {};
        for (string city : populations) {
            seatsAssigned[city]++;
            DataPoint data;
            data.name = city;
            data.weight = -(populations[city] / sqrt(2));
            numSeats--;
            que.enqueue(data);
        }

        while (numSeats != 0) {
            DataPoint data = que.dequeue();
            int ticket = ++seatsAssigned[data.name];
            data.weight = -(populations[data.name] / sqrt(ticket * (ticket + 1)));
            que.enqueue(data);
            numSeats--;
        }
