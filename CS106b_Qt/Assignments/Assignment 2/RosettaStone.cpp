...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    if (input.isEmpty() || allZero) {
        error("Error at normalize");
    }else {
        double sum = 0;
        for (string key : input.keys()) {
            sum += pow(input[key], 2);
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    if (numToKeep < 0) {
        ...
        if (hasExceeded) {
            ...
            Map<string, double> dst = source;
            int idx = 0;
            cnt -= numToKeep;
            while (idx < cnt) {
                dst.remove(que.dequeue());
                idx++;
            }
            return dst;
        }
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    double similarity = 0;
    for (string key1 : lhs.keys()) {
        for (string key2 : rhs.keys()) {
            if (key1 == key2) {
                similarity += lhs[key1] * rhs[key2];
            }
        }
    }
    return similarity;
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    if (corpora.isEmpty()) {
        error("Error at guessLanguageOf");
    } else {
        string lan = "";
        double similiar = 0;
        for (Corpus cor : corpora) {
            if (cosineSimilarityOf(textProfile, cor.profile) > similiar) {
                similiar =cosineSimilarityOf(textProfile, cor.profile);
                lan = cor.name;
            }
        }
        return lan;
