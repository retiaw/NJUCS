...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    output.fill(false);
    // initialize the queue with water_sources below or at the water_level:
    Queue<GridLocation> que;
    for (int i = 0; i < sources.size(); i++) {
        if (terrain[sources[i].row][sources[i].col] <= height) {
            output[sources[i].row][sources[i].col] = true;
            que.enqueue(sources[i]);
        }
    }
    // execute the queue (Breadth-First Search)
    while (!que.isEmpty()) {
        GridLocation target = que.dequeue();
        // scan the adjacent squeares of "target":
            // top":
        if (output.inBounds(target.row - 1, target.col)) {
            if (output[target.row - 1][target.col] == false && terrain[target.row - 1][target.col] <= height) {
                output[target.row - 1][target.col] = true;
                GridLocation loc;
                loc.row = target.row - 1;
                loc.col = target.col;
                que.enqueue(loc);
            }
        }
        ...
        if (output.inBounds(target.row + 1, target.col)) {
            ...
            // left:
        if (output.inBounds(target.row, target.col - 1)) {
            if (output[target.row][target.col - 1] == false && terrain[target.row][target.col - 1] <= height) {
                output[target.row][target.col - 1] = true;
                GridLocation loc;
                loc.row = target.row;
                loc.col = target.col - 1;
                que.enqueue(loc);
            }
        }
            // right:
