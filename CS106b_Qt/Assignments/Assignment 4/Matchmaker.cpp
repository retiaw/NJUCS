...
bool hasPaired(string person, const Set<Pair>& matching) {
    for (Pair group : matching) {
        if (person == group.first() || person == group.second()) {
            return true;
        }
    }
    return false;
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    if (possibleLinks.isEmpty()) {
        ...
        for (string person2 : possibleLinks[person1]) {
            if (!hasPaired(person2, matching)) {
                Pair group(person1, person2);
                Map<string, Set<string>> left_possibleLinks = possibleLinks;
                left_possibleLinks.remove(person1);
                left_possibleLinks.remove(person2);

                Set<Pair> new_matching = matching + group;
                if (hasPerfectMatching(left_possibleLinks, new_matching)) {
...
Set<Pair> maximumWeightMatching_helper(const Map<string, Map<string, int>> &possibleLinks,
                                       int weight)
{
    Set<Pair> matching = {};
    if (possibleLinks.isEmpty() || possibleLinks.size() == 1) {
        ...
        for (string person1 : possibleLinks.keys()) {
            for (string person2 : possibleLinks[person1].keys()) {
                if (possibleLinks.containsKey(person2) && possibleLinks[person1][person2] >= 0) {
                    if (new_weight < weight + possibleLinks[person1][person2]) {
                        new_weight = weight + possibleLinks[person1][person2];
                        Pair group(person1, person2);
                        Map<string, Map<string, int>> new_possibleLinks = possibleLinks;
                        new_possibleLinks.remove(person1);
                        new_possibleLinks.remove(person2);
                        matching = maximumWeightMatching_helper(new_possibleLinks, new_weight);
                        matching.add(group);
                    }
                }
            }
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    int max_weight = 0;
    return maximumWeightMatching_helper(possibleLinks, max_weight);
}
