...
void helper(Vector<string>& sentence, Set<string>& result, int idx)
{
    if (idx == sentence.size()) {
        ...
        if (!isalpha(sentence[idx][0])) {
            helper(sentence, result, idx + 1);
        } else {
            string ch1 = sentence[idx];
            sentence.set(idx, toLowerCase(sentence[idx]));
            helper(sentence, result, idx + 1);
            sentence.set(idx, ch1);

            string ch2 = sentence[idx];
            sentence.set(idx, toUpperCase(sentence[idx]));
            helper(sentence, result, idx + 1);
            sentence.set(idx, ch2);
        }
    }
}

Set<string> allEmphasesOf(const string& sentence) {
    Vector<string> target = tokenize(sentence);
    Set<string> result;
