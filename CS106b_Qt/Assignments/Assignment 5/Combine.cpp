...
Vector<DataPoint> Merge(const Vector<DataPoint> &vec1, const Vector<DataPoint> &vec2)
{
    Vector<DataPoint> vec;
    int i = 0, j = 0;

    while (i < vec1.size() && j < vec2.size()) {
        if (vec1[i].weight < vec2[j].weight) {
            vec.add(vec1[i]);
            i++;
        } else {
            vec.add(vec2[j]);
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    if (sequences.size() > 2) {
        ...
        Vector<DataPoint> vec1 = combine(sequences.subList(0, half));
        Vector<DataPoint> vec2 = combine(sequences.subList(half));
