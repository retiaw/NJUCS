...
bool RobinHoodHashTable::insert(const string& elem) {
    if (mySize == capacity || contains(elem)) {
        return false;
    } else {
        ...
        Slot temp = {elem, 0};
        while (1) {
            if (elems[(hash + hash_add) % capacity].distance == EMPTY_SLOT) {
                elems[(hash + hash_add) % capacity] = temp;
                mySize++;
                return true;
            } else if (elems[(hash + hash_add) % capacity].distance < temp.distance) {
                swap(temp, elems[(hash + hash_add) % capacity]);
            }
            hash_add++;
            temp.distance++;
        }
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    int cnt = 0;
    while (1) {
        if (elems[(hash + cnt) % capacity].value == elem && elems[(hash + cnt) % capacity].distance != EMPTY_SLOT) {
            return true;
        } else if (elems[(hash + cnt) % capacity].distance < cnt) {
            return false;
        } else {
            cnt++;
        }
...
bool RobinHoodHashTable::remove(const string& elem) {
    if (mySize == 0 || !contains(elem)) {
        ...
        while (1) {
            if (elems[(hash + cnt) % capacity].value == elem && elems[(hash + cnt) % capacity].distance != EMPTY_SLOT) {
                elems[hash + cnt].distance = EMPTY_SLOT;
                cnt++;
                break;
            }
            cnt++;
        ...
        // backshift:
        while (1) {
            if (elems[(hash + cnt) % capacity].distance == EMPTY_SLOT) {
                break;
            } else if (elems[(hash + cnt) % capacity].distance == 0)  {
                break;
            } else {
                elems[(hash + cnt - 1) % capacity].value = elems[(hash + cnt) % capacity].value;
                elems[(hash + cnt - 1) % capacity].distance = elems[(hash + cnt) % capacity].distance - 1;
                elems[(hash + cnt) % capacity].distance = EMPTY_SLOT;
                cnt++;
                // printDebugInfo();
            }
        }
