...
bool LinearProbingHashTable::insert(const string& elem) {
    if (contains(elem) || mySize == capacity) {
        return false;
    } else {
        int hash = hashCode(elem);
        int i = 0;
        while (elems[(hash + i) % capacity].type == SlotType::FILLED) {
            i++;
        }
        elems[(hash + i) % capacity].value = elem;
        elems[(hash + i) % capacity].type = SlotType::FILLED;
        mySize++;
...
bool LinearProbingHashTable::contains(const string& elem) const {
    int hash = hashCode(elem);
    int cnt = 0; // mod can ensure no escape of the capacity;
    while (cnt < capacity && elems[(hash + cnt) % capacity].type != SlotType::EMPTY) {
        if (elems[(hash + cnt) % capacity].value == elem && elems[(hash + cnt) % capacity].type == SlotType::FILLED) {
            return true;
        } else {
            cnt++;
        }
...
bool LinearProbingHashTable::remove(const string& elem) {
    if (!contains(elem) || mySize == 0) {
        return false;
    } else {
        int hash = hashCode(elem);
        int cnt = 0;
        while (1) {
            if (elems[(hash + cnt) % capacity].value == elem) {
                elems[(hash + cnt) % capacity].type = SlotType::TOMBSTONE;
                break;
            } else {
                cnt++;
            }
